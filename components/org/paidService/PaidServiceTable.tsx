...
type PaidServiceTableProps = {
  rows?: PaidService[];
  loading: boolean;
  onEdit: (row: PaidService) => void;
  onDelete: (row: PaidService) => void;
};
...
const PaidServiceTable = ({
  rows = [],
  loading,
  onEdit,
  onDelete,
}: PaidServiceTableProps) => {
  if (loading) {
  ...
  if (!rows.length) {
    return (
      <EmptyState
        title="No paid services found"
        description="Add a paid service to bill extras like transfers or corkage."
      />
    );
  }
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      aria-label={`Delete ${row.Service_Name}`}
                    >
...
export default PaidServiceTable;
